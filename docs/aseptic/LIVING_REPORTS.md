...
**What goes in:** Architectural directions documented but not yet implemented.
Concept-stage exploration that hasn't been built yet. The key test:
*"is this an architectural direction we want to pursue, and do we know when
implementation should begin?"*

**What does NOT go in:** Code that exists but should change (that's TECH_DEBT).
Code that's slightly imprecise (that's POLISH_DEBT). Spec/code divergence
(that's DEVIATION).

**ID prefix:** FD-NNN

**Entry body structure:** Same as TECH_DEBT entries (What it is / Why it's deferred /
Known cost / Trigger / Evidence), since future directions share the same "we know this
will be needed and we know when" structure.
