...
> **ID gap note:** TD-004 through TD-007 were originally drafted as TECH_DEBT entries
> but moved to `docs/aseptic/FUTURE_DIRECTIONS.md` because they describe research
> directions (concept-stage exploration) rather than functional-but-known-bad
> implementation choices. The Aseptic spec defines TECH_DEBT as the latter category.
> IDs are not reused per convention.
...
### TD-018 — CliRunner mix_stderr=False compat

**Surface:** Three test files share this failure pattern:
- `tests/cli/test_memory_cli.py` (29 failures)
- `tests/test_abstention.py` (3 failures)
- `tests/integration/test_memory_cli_against_vault.py` (7 failures)

Total: 39 failures, all sharing one root cause.

**Root cause:** Click removed the `mix_stderr=False` kwarg from `CliRunner()` in a newer
version. All affected test files use this kwarg via a shared helper pattern.

**Resolution:** Remove `mix_stderr=False` from `CliRunner()` instantiation calls across
affected files. If the helper is shared, single-point fix; if duplicated, multi-file edit.

**Trigger:** When Click version is next upgraded, or as standalone cleanup pass.

**Effort:** Small (single-line change × N files, possibly via single helper update).

**First observed:** Phase 9 Step 3 close (2026-06-13). Predates Phase 9 entirely.

---

### TD-019 — test_lattice_against_vault.py vault-disk failure

**Surface:** `tests/integration/test_lattice_against_vault.py` (1 failure).

**Root cause:** Unknown. Separate from TD-018 (not Click-related). Vault-against-disk test
failure pattern. Needs investigation.

**Resolution:** Investigate the failure mode; determine whether root cause is fixable or
whether the test should be marked skip with explanatory marker.

**Trigger:** Investigation work, or when adjacent vault test infrastructure changes.
