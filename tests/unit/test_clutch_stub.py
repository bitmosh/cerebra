...
from cerebra.cognition.clutch_stub import (
    BUILTIN_PREDICATES,
    ClutchContext,
    ClutchDecision,
    ClutchStubEngine,
...
class TestClutchStubEngine:
    def _engine(self, rules: list[dict]) -> ClutchStubEngine:
        cfg = _make_config_with_rules(rules)
        return ClutchStubEngine(cfg)  # type: ignore[arg-type]
    ...
    def test_no_match_returns_default_no_match(self) -> None:
        ...
        real_engine = ClutchStubEngine(cfg)
    ...
    def test_escalate_always_false_in_stub(self) -> None:
        engine = self._engine(
            [
                {
                    "name": "r",
